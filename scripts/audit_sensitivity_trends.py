...
def main() -> None:
    ...
    for parameter, expected in EXPECTED_TRENDS.items():
        ...
        for policy in ["RLBR", "SAA-BS-OBCA", "NVD", "DTP"]:
            ...
            row[f"{policy}_mismatch_first_last"] = (
                f"{policy_rows['mismatch_rate'].iloc[0]:.3f}->{policy_rows['mismatch_rate'].iloc[-1]:.3f}"
            )
...
def classify(parameter: str, winners: pd.DataFrame) -> tuple[str, str]:
    ...
    if parameter == "design_lead_time" and not winner_set.issubset({"RLBR", "SAA-BS-OBCA"}):
