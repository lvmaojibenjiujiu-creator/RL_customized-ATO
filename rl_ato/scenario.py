...
def make_instance(config: ExperimentConfig, seed: int | None = None) -> ProblemInstance:
    ...
    support, common_components, realized_commonality = _generate_product_component_graph(
        ...
        degree_min=int(config.family_degree_min),
        degree_max=int(config.family_degree_max),
        tolerance=float(config.commonality_tolerance),
        seed=seeds["graph"],
    )
    template_bom = _generate_template_bom(support, seeds["template_bom"])
    demand_lambdas = _rates_with_exact_sample_cv(
    ...
    initial_inventory = np.ceil(
        max(0.0, float(config.initial_inventory_factor))
        * _average_replenishment_lead_time(config)
        * template_component_demand
    )
    feature_scale = max(
        1.0,
    ...
    return ProblemInstance(
        ...
        config={
            ...
            "commonality_tolerance": float(config.commonality_tolerance),
...
def _generate_product_component_graph(
    ...
    degree_min = max(1, min(degree_min, J))
    degree_max = max(degree_min, min(degree_max, J, 6))
    jc = max(1, int(round(float(rho) * J)))
    effective_tol = max(float(tolerance), 0.5 / max(1, J) + 1e-12)
    best_support: np.ndarray | None = None
    best_common = np.arange(jc)
    best_gap = float("inf")
    best_realized = 0.0
    for _attempt in range(max_attempts):
        common_components = rng.choice(np.arange(J), size=jc, replace=False)
        common_mask = np.zeros(J, dtype=bool)
        common_mask[common_components] = True
        specific_components = np.flatnonzero(~common_mask)
        support = np.zeros((I, J), dtype=bool)
        for i in range(I):
            k_i = int(rng.integers(degree_min, degree_max + 1))
            k_common = min(jc, k_i, max(1, int(np.floor(float(rho) * k_i))))
            selected_common = rng.choice(common_components, size=k_common, replace=False)
            k_specific = k_i - k_common
            if k_specific > 0 and len(specific_components) > 0:
                n_specific = min(k_specific, len(specific_components))
                selected_specific = rng.choice(specific_components, size=n_specific, replace=False)
            else:
                selected_specific = np.array([], dtype=int)
            shortfall = k_i - k_common - len(selected_specific)
            if shortfall > 0:
                candidates = np.setdiff1d(common_components, selected_common, assume_unique=False)
                if len(candidates) < shortfall:
                    candidates = np.setdiff1d(np.arange(J), np.concatenate([selected_common, selected_specific]))
                extra = rng.choice(candidates, size=min(shortfall, len(candidates)), replace=False)
            else:
                extra = np.array([], dtype=int)
            support[i, np.concatenate([selected_common, selected_specific, extra])] = True

        for j in range(J):
            if not support[:, j].any():
                support[int(rng.integers(I)), j] = True
        for j in common_components:
            users = np.flatnonzero(support[:, j])
            if len(users) == 1 and I > 1:
                candidates = np.setdiff1d(np.arange(I), users)
                support[int(rng.choice(candidates)), j] = True

        realized = _realized_commonality(support)
        gap = abs(realized - float(rho))
        if gap < best_gap:
            best_gap = gap
            best_support = support.copy()
            best_common = common_components.copy()
            best_realized = realized
        if gap <= effective_tol and support.sum(axis=1).min() >= 1:
            return support, np.sort(common_components), realized
    assert best_support is not None
    return best_support, np.sort(best_common), best_realized
...
def _generate_template_bom(support: np.ndarray, seed: int) -> np.ndarray:
    rng = np.random.default_rng(seed)
    bom = np.zeros(support.shape, dtype=float)
    bom[support] = rng.integers(1, 4, size=int(support.sum()))
    return bom


def _rates_with_exact_sample_cv(
    rng: np.random.Generator, n: int, mean: float, cv: float, tolerance: float = 1e-4
) -> np.ndarray:
    if cv <= tolerance:
    ...
    def rates(scale: float) -> np.ndarray:
        r = np.exp(np.clip(scale * z, -60.0, 60.0))
        return mean * r / max(r.mean(), 1e-12)

    def sample_cv(scale: float) -> float:
        lam = rates(scale)
        return float(lam.std(ddof=0) / max(lam.mean(), 1e-12))

    lo, hi = 0.0, 1.0
    while sample_cv(hi) < cv and hi < 100.0:
        hi *= 2.0
    for _ in range(80):
        mid = 0.5 * (lo + hi)
        if sample_cv(mid) < cv:
            ...
            hi = mid
        if abs(sample_cv(mid) - cv) <= tolerance:
            break
    return rates(0.5 * (lo + hi))
