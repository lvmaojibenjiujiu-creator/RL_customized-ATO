...
class RLBRPolicy:
    ...
    def _adaptive_dtp_blend(self) -> float:
        ...
        if strength > 0.0:
            temporal_blend = float(np.clip(strength * max(window_easy, lead_easy), 0.0, 1.0))

        design_strength = float(getattr(self.config, "long_design_dtp_blend", 0.0))
        design_blend = 0.0
        if design_strength > 0.0:
            start = float(getattr(self.config, "long_design_blend_start", 3.0))
            width = max(1e-9, float(getattr(self.config, "long_design_blend_width", 4.0)))
            design_easy = max(0.0, (float(np.min(self.instance.design_lead_times)) - start) / width)
            design_blend = float(np.clip(design_strength * design_easy, 0.0, 1.0))
        return max(temporal_blend, design_blend)
    ...
    def _nvd_floor_target(
        ...
        known_requirement *= self._revealed_requirement_scale()
        return float(safety_scale) * self._ensure_nvd_base_stock() + known_requirement

    def _revealed_requirement_scale(self) -> float:
        scale = float(getattr(self.config, "revealed_requirement_scale", 1.0))
        high_cv_scale = float(getattr(self.config, "high_bom_cv_requirement_scale", 1.0))
        if abs(high_cv_scale - 1.0) > 1e-9:
            start = float(getattr(self.config, "high_bom_cv_start", 0.4))
            width = max(1e-9, float(getattr(self.config, "high_bom_cv_width", 0.1)))
            factor = float(np.clip((float(self.instance.bom_cv) - start) / width, 0.0, 1.0))
            scale *= (1.0 - factor) + factor * high_cv_scale
